  ...
  while (hasMore) {
    ...
    while (tryCount <= 4) {
      try {
        ...
        await new Promise((resolve, reject) => setTimeout(resolve, 5000 * 2 ** tryCount));
      }
    }
    ...
    offsetObject.after = Number.parseInt(searchResult?.paging?.next?.after);
  ...
  while (hasMore) {
    ...
    for (const contact of data) {
      ...
      const userProperties = {
        ...
        contact_score: Number.parseInt(contact.properties.hubspotscore) || 0,
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    if (!searchResult) {
      throw new Error('Failed to fetch meetings after multiple attempts.');
    }
    ...
    const associatedContactsByMeeting = await processAssociatedContactsByMeeting(meetingIds);
    const contactIds = Object.values(associatedContactsByMeeting);
    const contactEmails = await processContactsToAssociate(contactIds);

    for (const meeting of data) {
      if (!meeting.properties) continue;

      const isCreated = new Date(meeting.createdAt) > lastPulledDate;
      const contactId = associatedContactsByMeeting[meeting.id];
...
const processAssociatedContactsByMeeting = async (meetingIds) => {
  const associationsResult = (await (await hubspotClient.apiRequest({
    method: 'post',
    path: '/crm/v3/associations/MEETINGS/CONTACTS/batch/read',
    body: { inputs: meetingIds.map(meetingId => ({ id: meetingId })) }
  })).json())?.results || [];

  const meetingToContactAssociations = {};
  for (const assoc of associationsResult) {
    if (assoc.from && assoc.to && assoc.to.length > 0) {
      meetingToContactAssociations[assoc.from.id] = assoc.to[0].id;
    }
  }

  return meetingToContactAssociations
}

const processContactsToAssociate = async (contactIds) => {
  const contactEmailsResult = contactIds.length > 0 
  ...
  const contactEmails = {};
  for (const contact of contactEmailsResult.results) {
    if (contact.properties?.email) {
      contactEmails[contact.id] = contact.properties.email;
    }
  }
